...
function addPoiDetails(data, marker)
{
    ...
    let html = `
        <div class="flex flex-row items-center justify-between mb-2 mt-2">
...
function addPOItoDay(id, dayId, accommodation, visitDuration)
{
    ...
    if(day && !day.pois.find( ({poi}) => poi.id === id))
    {
        ...
        if('colours' in el)
        {
            ...
            markerDivEl.style.boxShadow = boxShadowString;
        }
        else
        {
            el['colours'] = [day.colour];
            markerDivEl.style.boxShadow = `0 0 0 3px ${el['colours'][0]}`;
            el['marker']._element.classList.add('z-20');
        }
        ...
        if(poiContainer.childNodes.length === 1 && poiContainer.childNodes[0].nodeType === Node.TEXT_NODE)
        {
            ...
            document.getElementById(`poiContainerParent_${day.id}`).style.maxHeight =
                document.getElementById(`poiContainerParent_${day.id}`).scrollHeight > 0 ?
...
function recalculateColours(el)
{
    if(('colours' in el) && el['colours'][0])
    {
        ...
        document.getElementById(`poi_marker_${el.poi.id}`).style.boxShadow = boxShadowString;
    }
    else
    {
